...
The baseline and current pre-physical build are valid ESP32-S3 application images. They prove the starting platform and current safety integration compile, not that CJK changes are hardware-safe. The current application is normalized from a stripped temporary ELF so debug paths do not change its app-descriptor hash across worktrees. It is not published as installable because it lacks physical display, reader, Anki, and recovery evidence, and it has zero bytes of free IRAM. The checked-in source-side verifier record is `site/release-manifest.json`; it deliberately carries `installable: false`.
