...
| Pre-physical source commit | 51fc0470d54c3217fb35fc2b16cb621b35e290a5 |
| Pre-physical app image | 5,390,320 bytes |
| Pre-physical app SHA-256 | 60457af20c3d9e16f96c4853af08f8e11866dfd6d7bed7c1446401aef2a1062b |
| Pre-physical build checks | ESP32-S3 header, valid checksum/hash, one X4 Pro tag, 222 host tests, 14 Python build/release checks, artifact validation, and local SSD1677/UC8179/UC8279 simulator build-and-Home captures |

The baseline and current pre-physical build are valid ESP32-S3 application images. They prove the starting platform and current safety integration compile, not that CJK changes are hardware-safe. The current application is normalized from a stripped temporary ELF so debug paths do not change its app-descriptor hash across worktrees. The size report's 16 KiB dedicated-IRAM region is fully assigned, while the shared D/IRAM budget has 184,682 bytes after static allocation; runtime heap is lower after SDK startup and remains unmeasured. It is not published as installable because it lacks physical display, reader, Anki, and recovery evidence. The checked-in source-side verifier record is `site/release-manifest.json`; it deliberately carries `installable: false`.
